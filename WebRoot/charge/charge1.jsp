...
<div id=win style="display: none; position: absolute; left: 50%; top: 40%; width: 450px; height: 340px; margin-left: -300px; margin-top: -200px; border: 1px solid #888; background-color: gray; text-align: center;">
     ...
     <span>收费编号:</span><input id="ch_charName"></input><br><br>        
     <span>房&nbsp;&nbsp;&nbsp;&nbsp;间:</span><input id="ch_roomNum"></input><br><br>   
     <span>上次缴费:</span><input id="ch_lasDate" onClick="WdatePicker()"></input> <br> <br>
     <span>截止时间：</span><input id="ch_date" onClick="WdatePicker()"></input><br><br>
     <span>收费金额：</span><input id="ch_amount" ></input><br><br>
     <span>收费类型:</span><input id="ch_type"></input> <br>
...
function openEdit(id){  
	...
	var num=doc.getElementById("chargeNum"+id).innerHTML;	
	var room=doc.getElementById("comName"+id).innerHTML;
	var type=doc.getElementById("cahrgeType"+id).innerHTML;
	var price=doc.getElementById("chargePrice"+id).innerHTML;
	var lastdate=doc.getElementById("chargeLasDate"+id).innerHTML;
	var date=doc.getElementById("chargeDate"+id).innerHTML;
	doc.getElementById("ch_charName").value=num;
	doc.getElementById("ch_roomNum").value=room;
	doc.getElementById("ch_lasDate").value=lastdate;
	doc.getElementById("ch_date").value=date;
	doc.getElementById("ch_amount").value=price;
	doc.getElementById("ch_type").value=type;
...
function jumpTo(){
	 ...
	 if(temp<=count && temp>0)
	  	window.location.href="../servlet/Charge1?jump_type=jump_to&to_page="+temp;
}
function closeLogin(){   
...
function addSured(){  
	 ...
	 if(isSureAdded==true){
	     ...
	     addForm.action="../servlet/charge2_Insert";
	     addForm.submit(); 
	     closeAdd();    
	 }
	 else
...
function editSured(){ 
    ...
    {
	   var updateCharName=doc.getElementById("ch_charName").value;
	   var updateRoomNum=doc.getElementById("ch_roomNum").value;
	   var updateDate=doc.getElementById("ch_date").value;
	   var updateLasDate=doc.getElementById("ch_lasDate").value;
	   var updateAmount=doc.getElementById("ch_amount").value;
	   var updateType=doc.getElementById("ch_type").value;
	   doc.getElementsByName("charNumValue")[0].value=updateCharName;
	   doc.getElementsByName("roomValue")[0].value=updateRoomNum;
	   doc.getElementsByName("dateValue")[0].value=updateDate;
	   doc.getElementsByName("lasDateValue")[0].value=updateLasDate;
	   doc.getElementsByName("amountValue")[0].value=updateAmount;
	   doc.getElementsByName("typeValue")[0].value=updateType;
	   ...
	   form.action="../servlet/charge2_Update";
	   form.submit();
	   closeLogin();
   }
   else
...
function delSingleItem(delId){ 
    ...
    if(isSureDeled){
	    ...
	    form.action="../servlet/charge2_Delete";
	    form.submit();   
    }//if         
}
//批量删除函数 2015.7.13
function delBatItems(){ 
   ...
   keyform.action="../servlet/charge2_Delete";
   keyform.submit(); 
   } 
...
<form name="transForm" method="post" action="">
	...
	<input type="hidden" name="charNumValue" value=""/>   
	<input type="hidden" name="roomValue" value=""/>   
	<input type="hidden" name="lasDateValue" value=""/>   
	<input type="hidden" name="dateValue" value=""/>   
	<input type="hidden" name="amountValue" value=""/>  
...
<br>
<form action="../servlet/Charge1_Select" method="post" name="queryform">
			<table class="gridtable" width="98%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td>
							<font size=2 style="vertical-align: middle">收费编号:</font>
							<input type="text" id="charge_num" name="charge_num"></input>
						</td>
					</tr>
													...
													<tr>
														...
														<%
														      if(((Integer)session.getAttribute("currentPage")).intValue()!=0){												    	   
														    	  out.print("<a href=\"../servlet/Charge1?jump_type=first\">");
														      }												    
														%>										
														...
														<td width="45">													    
														    ...
														    <%
														        if(((Integer)session.getAttribute("currentPage")).intValue()>=2){												    	   
														    	    out.print("<a href=\"../servlet/Charge1?jump_type=back\">");
														    	}												    
														    %>														  
														...
														<td width="45">
														    <div align="center">
														     ...
														     if(((Integer)session.getAttribute("currentPage")).intValue()<((Integer)session.getAttribute("pageCount")/15+((Integer)session.getAttribute("pageCount")%15!=0?1:0))){							    	    
														    	 out.print("<a href=\"../servlet/Charge1?jump_type=next\">");
														     }									  											    
														    %>													  													   	
														...
														<%
														     if(((Integer)session.getAttribute("currentPage")).intValue()!=0){												    	   
														    	 out.print("<a href=\"../servlet/Charge1?jump_type=last\">");
														     }												    
														%>										
