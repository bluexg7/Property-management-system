...
function jumpTo(){
	 ...
	 if(temp<=count&&temp>0)
	  	window.location.href="../servlet/Security2?jump_type=jump_to&to_page="+temp;
}
function closeLogin(){   
...
function submit(){
 	var form=doc.forms("transform");  //对应第一个table form
   	form.action="../servlet/Security2_Update";
    form.submit();
}
...
function addSured(){  
	 ...
	 if(isSureAdded==true){
	    ...
	    addForm.action="../servlet/Security2_Insert";
	    addForm.submit(); 
	    closeAdd();    
	 }
	 else
...
function delSingleItem(delId){ 
    ...
    if(isSureDeled){
	    ...
	    form.action="../servlet/Security2_Delete";
	    form.submit();   
    }//if         
}
function resetAll(){
...
function delBatItems(){ 
   ...
   keyform.action="../servlet/Security2_Delete";
   keyform.submit();  
}
</script>
...
<div class="box1">&nbsp;
	...
	<font color="blue" size=2pt>[保安管理]-[日常安保记录]<br ></br> </font>
</div>
<br>
 <form action="../servlet/Security2_Select" method="post" name="queryform">
			<table class="gridtable" width="98%" border="0" cellspacing="0"
					cellpadding="0">
					<tr>
						<td>
							<font size=2>&nbsp;&nbsp;保安序号:</font>
					   ...
					   <td>
							...
							out.print(session.getAttribute("work_time_tran"));session.setAttribute("work_time_tran","");%>
													...
													<tr>
														...
														<%
														      if(((Integer)session.getAttribute("currentPage")).intValue()!=0){												    	   
														    	  out.print("<a href=\"../servlet/Security2?jump_type=first\">");
														      }												    
														%>										
														...
														<td width="45">													    
														    ...
														    <%
														        if(((Integer)session.getAttribute("currentPage")).intValue()>=2){												    	   
														    	    out.print("<a href=\"../servlet/Security2?jump_type=back\">");
														    	}												    
														    %>														  
														...
														<td width="45">
														    <div align="center">
														     ...
														     if(((Integer)session.getAttribute("currentPage")).intValue()<((Integer)session.getAttribute("pageCount")/15+((Integer)session.getAttribute("pageCount")%15!=0?1:0))){							    	    
														    	 out.print("<a href=\"../servlet/Security2?jump_type=next\">");
														     }									  											    
														    %>													  													   	
														...
														<%
														     if(((Integer)session.getAttribute("currentPage")).intValue()!=0){												    	   
														    	 out.print("<a href=\"../servlet/Security2?jump_type=last\">");
														     }												    
														%>										
