...
public class Clean2 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<Clean2_Info> resultList = c1_dao.select(cleaner_num,work_content,tools,work_time);
		...
		//查询结果的一页
		List<Clean2_Info> list = resultList.subList((currentPage-1)*15,(currentPage*15<resultList.size()-1)?currentPage*15:resultList.size());
	    System.out.println("共查到日常保洁记录"+list.size()+"条");
	    System.out.println("********************************");
        request.setAttribute("dailyclean_list", list);
		request.getRequestDispatcher("../clean/clean2.jsp").forward(request,response);
