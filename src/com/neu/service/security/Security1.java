...
public class Security1 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<Security1_Info> resultList = c1_dao.select(cleaner_num,name,position,work_range);
		//查询结果
		...
		//查询结果的一页
		List<Security1_Info> list = resultList.subList((currentPage-1)*15,(currentPage*15<resultList.size()-1)?currentPage*15:resultList.size());
		
		request.setAttribute("list",list);   
		request.getRequestDispatcher("../security/security1.jsp").forward(request,
				response);
